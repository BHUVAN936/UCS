from flask import Blueprint, render_template

from services.data_service import (
    MODULES,
    build_module_report,
    get_module_data,
    prepare_module_tables,
)


faculty_fdp_corporate_bp = Blueprint(
    "faculty_fdp_corporate",
    __name__,
    url_prefix="/faculty-fdp-corporate"
)


@faculty_fdp_corporate_bp.route("/")
def index():

    module_key = "faculty_fdp_corporate"

    grouped_data = get_module_data(module_key)

    report = build_module_report(
        module_key,
        grouped_data
    )

    tables = prepare_module_tables(
        grouped_data
    )

    return render_template(
        "faculty_fdp_corporate/index.html",
        module_key=module_key,
        module_info=MODULES[module_key],
        subtopics=MODULES[module_key]["categories"],
        grouped_data=grouped_data,
        report=report,
        tables=tables,
    )